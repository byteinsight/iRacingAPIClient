#
#    Copyright (c) 2022 Chris Davies <chris@byteinsight.co.uk>
#
...
class iRacingClient():
    ...
    def rawToJson(self, export_path, raw_data):
    ...
    def get_cars_assets(self, export=False):
        ...
        if export:
            export_path = os.path.join(self.hive_root, self.files_folder, 'cars_assets.json')
    ...
    def get_cars(self, export=False):
        ...
        if export:
            export_path = os.path.join(self.hive_root, self.files_folder, 'cars.json')
            self.rawToJson(export_path, raw_data)
        return raw_data
    ...
    def get_league_directory(self, search="", tag="", restrict_to_member=False, restrict_to_recruiting=False,
        ...
        if export:
            export_path = os.path.join(self.hive_root, self.files_folder, 'league_directory.json')
            self.rawToJson(export_path, raw_data)
        return raw_data
    ...
    def member_chart_data(self, cust_id=None, category_id=2, chart_type=1, export=False):
        ...
        if export:
            ...
            export_path = os.path.join(self.hive_root, self.files_folder, filename)
            self.rawToJson(export_path, raw_data['data'])
        return raw_data
    ...
    def result_event_log(self, subsession_id=None, simsession_number=0, export=False):
        ...
        if export:
            filename = "result_event_log" + str(subsession_id) + ".json"
            export_path = os.path.join(self.hive_root, self.files_folder, filename)
            chunks = self._get_chunks(raw_data["chunk_info"])
            self.rawToJson(export_path, chunks)

        return chunks

    # lap_chart_data
    def result_lap_chart_data(self, subsession_id=None, simsession_number=0, cust_id=None, team_id=None, export=False):
        """Returns a dictionary of the lap data for the given sub session id"""
    ...
    def get_series(self, export=False):
        ...
        if export:
            export_path = os.path.join(self.hive_root, self.files_folder, 'series.json')
            self.rawToJson(export_path, raw_data)
        return raw_data
    ...
    def series_seasons(self, include_series=False, export=False):
        ...
        if export:
            export_path = os.path.join(self.hive_root, self.files_folder, 'series_seasons.json')
            self.rawToJson(export_path, raw_data)
        return raw_data
    ...
    def stats_member_career(self, cust_id=None, export=False):
        ...
        if export:
            ...
            export_path = os.path.join(self.hive_root, self.files_folder, filename)
            self.rawToJson(export_path, raw_data['stats'])
        return raw_data
    ...
    def member_recent_races(self, cust_id=None, export=False):
        ...
        if export:
            ...
            export_path = os.path.join(self.hive_root, self.files_folder, filename)
            self.rawToJson(export_path, raw_data['races'])
        return raw_data
    ...
    # assets
    def get_tracks_assets(self, export=False):
        try:
        ...
        if export:
            export_path = os.path.join(self.hive_root, self.files_folder, 'tracks_assets.json')
            self.rawToJson(export_path, raw_data)
        return raw_data
    ...
    def get_tracks(self, export=False):
        ...
        if export:
            export_path = os.path.join(self.hive_root, self.files_folder, 'tracks.json')
            self.rawToJson(export_path, raw_data)
        return raw_data
